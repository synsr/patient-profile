...
export const getPatientAlerts = async (id: string): Promise<AlertsResponse> => {
  await delay(500);
  // Filter alerts for this patient
  const filteredAlerts = (alertsData.data as Alert[]).filter((alert) => alert.patient.id === id);
  return {
    data: filteredAlerts,
